...
public class EvilRadioUserSnapshotProcessor extends EntitySnapshotProcessor<Player> {

  public EvilRadioUserSnapshotProcessor(EntitySnapshotRegistry registry) {
    super(registry);
  }
  ...
  public void process(Player player, float partialTicks, ExtrasWriter entityWriter) {
    this.registry().captureSnapshot(entityWriter, EvilRadioKeys.USER, player);
  }
}
